...
| json   | json validation  will allow only Json to be entered.<br/> Example(You can use any of below two based on your need) : <br/> 1. Validator Based : ```RxwebValidators.json({message:'Enter the text in JSON format --> {key:value}' })``` <br/> 2. Decorator Based : ```@json({message:'Enter the text in JSON format --> {key:value}' }) locationJson: string;```
