...
message and conditional expression options are not mandatory to use in the `@minLength()`  {{validatorType}}  but value is mandatory. If needed then use the below options.
...
<tr><td><a  (click)='scrollTo("#message")' title="message">allowWhiteSpace</a></td><td>To override the global configuration message and show the custom message on particular control property.</td></tr>
</table>
